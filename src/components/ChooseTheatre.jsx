...
class ChooseTheatre extends React.Component{
    ...
    handleFilter=()=>{
        ...
        filteredMovie=movies.filter(e=>e.id==selectedMovieId)[0]
        filteredMovie=filteredMovie.theatres.filter(e=>e.location.toLowerCase()==location.toLowerCase())
    ...
    render(){
        const {isLogged,movies,selectedMovieId,setSelectedTheatreId}=this.props
        return(
            <>
            <div className="d-flex">
            <div style={{height:this.state.theatresData.length < 2 ? "100vh" : "145vh" }} className="col-3 bg-danger">
            ...
            <div  className="col row mt-2 mr-3 offset-2">
                {selectedMovieId && this.state.theatresData.length > 0 && this.state.theatresData.map(e=>{
              return (
                <Card key={uuid()} className="m-2" style={{ width: "300px" }}>
                  ...
                  <CardActionArea className="text-center">
                   <Link to={`/chooseseats:${e.id}`}>
                      <Button onClick={()=>setSelectedTheatreId(e.id)} size="small" color="primary">
                        Select this theatre
                      </Button>
                    </Link> 
                  </CardActionArea>
