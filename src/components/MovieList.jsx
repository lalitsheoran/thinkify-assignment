...
class MovieList extends React.Component {
  ...
  handleFilter = () => {
    ...
    if (this.state.filterx == "all") {
      ...
      this.state.filterx != "all" &&
      this.state.filterType == "genre"
    ) {
      dataFiltered = this.props.movies.filter(
        (e) => e.genre == this.state.filterx
      );
      this.setState({
        data: dataFiltered,
      });
      ...
      this.state.filterx != "all" &&
      this.state.filterType == "language"
    ) {
      dataFiltered = this.props.movies.filter(
        (e) => e.language == this.state.filterx
      );
      this.setState({
        data: dataFiltered,
      });
    ...
    return (
      <>
        <div className="d-flex">
          <div
            style={{ height: this.state.data.length < 4 ? "100vh" : "175vh" }}
          ...
          >
            ...
            <Link to="/mybookings">
              <p className="text-white h4 text-center mt-5 border py-2">
                Go to your bookings
              </p>
            </Link>
            ...
            <div>
              <button
                ...
                type="button"
                class=" mt-5 btn btn-outline-light"
              >
                Logout
              </button>
            </div>
                ...
                return (
                  <Card key={uuid()} className="m-2" style={{ width: "220px" }}>
                    ...
                    <CardActionArea className="text-center">
                      <Link to={`/book:${e.id}`}>
                        <Button
                          onClick={() => setSelectedMovieId(e.id)}
                          size="small"
                          color="primary"
                        >
                          Book this movie
